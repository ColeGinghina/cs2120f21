...
begin
  ...
  exact eq.refl ff, -- or exact rfl,
end

example : (∃ (b : bool), b && tt = ff) → (∃ (b : bool), true) :=
begin
  assume h,
  -- exact exists.intro true true.intro, <-- this would finish the proof as well
  cases h with w pf,
  apply exists.intro w,
  trivial, -- or apply true.intro,
end
